...
static int sf32lb52_lsm6ds3_initialize(FAR struct i2c_master_s *i2c)
{
  ...
  /* DevKit-LCD: PA30 is the panel TOUCH I2C1 SCL and PA31 is the panel
   * TOUCH IRQ, so this sensor glue must not re-purpose them (it would
   * leave the touch bus dead).  The board has no LSM6DS3; just let the
   * register probe below fail.
   */
...
/* This board wires CTP_INT to PA31; the shipped defconfig still carries
 * CONFIG_TOUCH_IRQ_PIN=41 because it was copied from the LCKFB Huangshan Pi
 * board, where CTP_INT really is PA41 (see its bsp_pinmux.c BSP_PIN_Touch).
 * Override locally so the FT6146 interrupt lands on the pin that is actually
 * routed on this PCB.
 */
...
int sf32lb52_lchspi_ulp_bringup(void)
{
  ...
  /* DevKit-LCD: touch I2C1 SCL is PA30.  PA37 is LCD_VADD_EN
   * (CO5300 AMOLED VADD rail) and must stay a plain GPIO, so the
   * PA37 -> I2C1_SCL remap copied from the -ULP board is removed
   * here: PA37 must stay LCD_VADD_EN.
   */
  ...
  /* 板载音频通路 bring-up：三段式自检（静音 / 1kHz / 音阶）。
   * 背景见 bsp_audio_test.c 头部注释。当成任务起，别拖慢 bringup。 */
  {
    extern int bsp_audio_selftest_start(void);
